...
import { UseThemeContext, DefaultProps, ProviderProps, ThemeState } from "./provider.types";
...
const Provider: FC<ProviderProps> = ({
  ...
  const handleChange = useCallback(
    (theme: string, type?: HandlerTypes) => {
      if (theme === "system") {
        setThemeState("system", type);
      } else if (themes.includes(theme)) {
        setThemeState(theme, type);
      } else {
        console.error(
          `Unknown theme: ${theme}. Have you included it in the themes prop?`
        );
        // We default to system theme if no previous theme is active
        if (themeState.resolvedTheme === null) {
          setThemeState("system", type);
        }
      }
    },
    [darkTheme, lightTheme, matches, themeState, themes]
  );
  ...
  const handleProviderChange = useCallback(
    (theme: string | null, type: HandlerTypes) => {
      if (respectHandlerOrder) {
        ...
        if(respected.resolvedTheme) handleChange(respected.resolvedTheme, type)
  ...
  const providerValue: UseThemeContext = useMemo(
    ...
    [handleChange, themeState, themes]
  );
